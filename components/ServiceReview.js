        ...
        <div className="mt-4 bg-white shadow h-96 flex flex-col overflow-hidden">
            ...
            <div className="flex">
                <input onChange={handleInputChange}
                       className={`${signedIn ? "inline-block" : "hidden"} shadow text-gray-600 flex-grow bg-white h-10 px-5 pr-16 text-sm focus:outline-none`}
                       type="text" placeholder="Enter your review..." name="review"/>
                <button onClick={() => submitReview()}
                        className={`${signedIn ? "inline-block" : "hidden"} focus:outline-none bg-primary text-sm text-white font-display px-4 py-2`}>Post
                </button>
                <p className={`${signedIn ? "hidden" : "inline-block"} w-full text-center py-2 border text-xs mt-2 text-red-500 text-center`}>Please
                    sign in to use this feature</p>
            </div>
        </div>
    )
}
